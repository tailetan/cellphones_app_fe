...
function Details() {
  ...
  React.useEffect(() => {
    ...
    handleReview(window.location.pathname.substring(window.location.pathname.lastIndexOf('/') + 1));
    return () => {
  ...
  const handleReview = async (id) => {
    const result = await axios.get(`/product/list-review/${id}`);
    ...
    <Theme>
      <div>
        ...
        <main style={{ paddingTop: '64px', position: 'relative' }}>
          {Object.keys(details).length === 0 && <img src={NotFound} width="100%" />}
            ...
            <>
              ...
              <div className="container p-x-0">
                <div className="d-flex align-items-center m-y-6">
                  <h4 className="font-weight-bold m-r-6">{details.product_name}</h4>
              ...
              <div className="container" style={{ position: 'relative' }}>
                <Row>
                  <Col xs={4}>
                    {details.product_images && details?.product_images !== null ? (
                      <Slider images={details.product_images} />
                    ) : (
                      <div style={{ border: '1px solid #d1d5db', borderRadius: '15px' }}>
                          ...
                          {new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' })
                            .format(
                              details.product_price -
                                details.product_price * parseFloat(details.discount_percent)
                            )
                            .toString()}
                        </p>
                      </div>
